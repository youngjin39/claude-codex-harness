...
main() {
  ...
  # Add --deselect flags here for any known-flaky tests in your repo.
  # Example: known_flake_deselect="--deselect tests/test_slow_live.py::test_case"
  local known_flake_deselect
  known_flake_deselect=""
  if [ -n "$changed_py_under_src" ]; then
    default_test="uv run pytest -q${known_flake_deselect:+ $known_flake_deselect}"
